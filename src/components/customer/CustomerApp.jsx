    ...
    try {
      ...
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-sonnet-4-20250514',
          max_tokens: 4000,
          messages: [{ role: 'user', content: 'Translate these UI strings for an Ibiza delivery app into ' + (langNames[newLang]||newLang) + '. Keep brand names (Isla Drop, Ibiza, Aperol, Prosecco, Moet, Red Bull, etc). Keep numbers and currency symbols. Return ONLY a valid JSON object with the same keys. No markdown, no explanation.\n\n' + list }]
        })
      })
      if (resp.ok) {
        const data = await resp.json()
