...
function LangSheet({ lang, setLang, onClose }) {
  return (
    <>
      <div onClick={onClose} style={{ position:'fixed', top:0, left:0, right:0, bottom:0, zIndex:9000, background:'rgba(0,0,0,0.6)' }} />
      <div style={{ position:'fixed', left:0, right:0, bottom:0, zIndex:9001, background:'#0D3545', borderRadius:'18px 18px 0 0', paddingBottom:36 }}>
        ...
        <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.4)', textTransform:'uppercase', letterSpacing:'1px', padding:'6px 16px' }}>Select language</div>
        <div style={{ overflowY:'auto', maxHeight:'50vh' }}>
          {LANGUAGES.map(l=>(
            <div key={l.code} onClick={()=>{setLang(l.code);onClose()}}
              style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 16px', cursor:'pointer', background:l.code===lang?'rgba(196,104,58,0.2)':'transparent', borderTop:'0.5px solid rgba(255,255,255,0.07)', fontFamily:'DM Sans,sans-serif', fontSize:14, color:'white' }}>
