...
// ── Language Picker ───────────────────────────────────────────
function LanguagePicker({ lang, onOpen }) {
  const cur = LANGUAGES.find(l=>l.code===lang)||LANGUAGES[0]
  return (
    <button onClick={onOpen} style={{ background:'rgba(255,255,255,0.14)', border:'0.5px solid rgba(255,255,255,0.22)', borderRadius:20, padding:'5px 11px', color:'white', fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:5, fontFamily:'DM Sans,sans-serif' }}>
      <span style={{ fontSize:14 }}>{cur.flag}</span>{cur.code.toUpperCase()}
    </button>
  )
}

function LangSheet({ lang, setLang, onClose }) {
  return (
    <>
      <div onClick={onClose} style={{ position:'fixed', top:0, left:0, right:0, bottom:0, zIndex:9000, background:'rgba(0,0,0,0.55)' }} />
      <div style={{ position:'fixed', left:0, right:0, bottom:0, zIndex:9001, background:'#0D3545', borderRadius:'18px 18px 0 0', maxHeight:'45vh', overflowY:'auto', WebkitOverflowScrolling:'touch' }}>
        <div style={{ position:'sticky', top:0, background:'#0D3545', paddingBottom:4, zIndex:1 }}>
          <div style={{ textAlign:'center', padding:'10px 0 4px' }}><div style={{ width:32, height:4, background:'rgba(255,255,255,0.2)', borderRadius:2, display:'inline-block' }}/></div>
          <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.4)', textTransform:'uppercase', letterSpacing:'1px', padding:'4px 16px 8px' }}>Language</div>
        </div>
        {LANGUAGES.map(l=>(
          <div key={l.code}
            onClick={(e)=>{ e.stopPropagation(); setLang(l.code); onClose() }}
            style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:l.code===lang?'rgba(196,104,58,0.2)':'transparent', borderTop:'0.5px solid rgba(255,255,255,0.07)', fontFamily:'DM Sans,sans-serif', fontSize:14, color:'white', cursor:'pointer' }}>
      ...
      <div style={{ background:C.header }}>
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 16px 0' }}>
          ...
          <div style={{ display:'flex',gap:7,alignItems:'center' }}>
            ...
            <LanguagePicker lang={lang} onOpen={()=>setLangOpen(true)} />
          </div>
        </div>
...
export default function CustomerApp() {
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', paddingBottom:68 }}>
      ...
      {langOpen && <LangSheet lang={lang} setLang={setLang} onClose={()=>setLangOpen(false)} />}
