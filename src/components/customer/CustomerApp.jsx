          ...
          <button key={tab.id} onClick={()=>setView(tab.id)}
            ...
            <div style={{ position:'relative' }}>
              <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={on?2:1.7}>
                {tab.search ? <><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></> : <path d={tab.path}/>}
              </svg>
...
function BasketView({ t, onCheckout }) {
  ...
  if (cart.getItemCount()===0) return (
    <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:40,minHeight:'60vh' }}>
      ...
      <div style={{ fontFamily:'DM Serif Display,serif',fontSize:22,color:'rgba(255,255,255,0.85)',marginBottom:6 }}>{t.yourBasketEmpty}</div>
  ...
  return (
    <div style={{ padding:'16px 16px 20px' }}>
      ...
      <div style={{ marginTop:16,padding:'14px 0',borderTop:'0.5px solid rgba(255,255,255,0.1)' }}>
        <div style={{ display:'flex',justifyContent:'space-between',fontSize:13,color:'rgba(255,255,255,0.5)',marginBottom:5 }}><span>{t.subtotal}</span><span>€{cart.getSubtotal().toFixed(2)}</span></div>
...
function AccountView({ t }) {
  ...
  return (
    <div style={{ padding:'20px 16px' }}>
      <div style={{ fontFamily:'DM Serif Display,serif',fontSize:26,color:'white',marginBottom:20 }}>{t.account}</div>
      {user ? (
        <>
          ...
          {[['📦',t.orderHistory||'Order history'],['📍',t.savedAddresses||'Saved addresses'],['🔔',t.notifications||'Notifications'],['🌍','Language & region'],['❓','Help & support']].map(([icon,label])=>(
        ...
        <div style={{ textAlign:'center',padding:'40px 0' }}>
          ...
          <div style={{ fontFamily:'DM Serif Display,serif',fontSize:22,color:'white',marginBottom:8 }}>{t.signInTitle}</div>
          <div style={{ fontSize:14,color:'rgba(255,255,255,0.45)',marginBottom:24 }}>Track orders, save addresses, and more</div>
          <button onClick={()=>toast('Use the sign in option')} style={{ padding:'13px 32px',background:'#C4683A',color:'white',border:'none',borderRadius:12,fontFamily:'DM Sans,sans-serif',fontSize:14,cursor:'pointer' }}>{t.signIn}</button>
        </div>
      )}
    </div>
  )
...
function CategoriesView({ onSelect }) {
  return (
    <div style={{ padding:'16px 16px 0' }}>
      <div style={{ fontFamily:'DM Serif Display,serif',fontSize:26,color:'white',marginBottom:16 }}>{t.categories}</div>
        ...
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 16px 0' }}>
          ...
          <div style={{ display:'flex',gap:7,alignItems:'center' }}>
            <div style={{ background:'rgba(255,255,255,0.12)',border:'0.5px solid rgba(255,255,255,0.18)',borderRadius:20,fontSize:11,padding:'4px 10px',display:'flex',alignItems:'center',gap:5,color:'white' }}>
              <span style={{ width:5,height:5,borderRadius:'50%',background:'#7EE8A2',display:'inline-block',animation:'pulse 1.5s infinite' }}/>{t.open247||'Open 24/7'}
            </div>
    ...
    return (
      <div style={{ background:C.bg, minHeight:'100vh', paddingBottom:60 }}>
        <div style={{ padding:'16px 16px 20px' }}>
          ...
          <div style={{ fontFamily:'DM Serif Display,serif',fontSize:16,color:'white',marginBottom:12 }}>{t.deliveryLocation}</div>
          <div style={{ borderRadius:14,overflow:'hidden',marginBottom:20 }}>
            <DeliveryMap onLocationSet={()=>setLocationSet(true)} />
          </div>
          <div style={{ fontFamily:'DM Serif Display,serif',fontSize:16,color:'white',marginBottom:12 }}>{t.orderSummary}</div>
          <div style={{ background:'rgba(255,255,255,0.07)',borderRadius:14,padding:16,marginBottom:20 }}>
            ...
            <div style={{ borderTop:'0.5px solid rgba(255,255,255,0.1)',paddingTop:10,marginTop:6 }}>
              <div style={{ display:'flex',justifyContent:'space-between',fontSize:13,color:'rgba(255,255,255,0.45)',marginBottom:5 }}><span>{t.subtotal}</span><span>€{cart.getSubtotal().toFixed(2)}</span></div>
          ...
          <div style={{ fontFamily:'DM Serif Display,serif',fontSize:16,color:'white',marginBottom:16 }}>{t.payment}</div>
      ...
      <div style={{ background:C.bg, minHeight:'100vh', padding:'24px 16px 100px' }}>
        ...
        {activeOrder.estimated_minutes&&activeOrder.status!=='delivered'&&(
          <div style={{ background:'rgba(43,122,139,0.2)',border:'0.5px solid rgba(43,122,139,0.35)',borderRadius:14,padding:'14px 16px',marginBottom:20,display:'flex',alignItems:'center',gap:12 }}>
            ...
            <div><div style={{ fontSize:12,color:'rgba(255,255,255,0.45)' }}>{t.estimatedArrival}</div><div style={{ fontSize:22,fontWeight:500,color:'white' }}>{activeOrder.estimated_minutes} min</div></div>
          </div>
        )}
        {/* Live tracking map */}
        {activeOrder.driver_id && (
          <div style={{ marginBottom:16 }}>
            <div style={{ fontFamily:'DM Serif Display,serif',fontSize:16,color:'white',marginBottom:10 }}>{t.liveTracking}</div>
            <OrderTrackingMap order={activeOrder} driverId={activeOrder.driver_id} />
          </div>
        )}
        <button onClick={()=>setView(VIEWS.HOME)} style={{ width:'100%',padding:15,background:'#C4683A',color:'white',border:'none',borderRadius:12,fontFamily:'DM Sans,sans-serif',fontSize:15,cursor:'pointer' }}>{t.placeAnotherOrder}</button>
      </div>
    )
  }
