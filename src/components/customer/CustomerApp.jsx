...
function AccountView({ t }) {
  ...
  const handleSupport = async (text) => {
    ...
    if (!q) return
    setSupportMessages(prev => [...prev, { role:'user', text: text || supportInput }])
    setSupportInput('')
  ...
  if (showSupport) return (
    <div style={{ padding:'16px 16px 20px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
        <button onClick={()=>setShowSupport(false)} style={{ background:'none',border:'none',color:'rgba(255,255,255,0.6)',cursor:'pointer',fontSize:20,padding:0 }}>←</button>
        <div style={{ fontFamily:'DM Serif Display,serif',fontSize:22,color:'white' }}>Customer Support</div>
      </div>
      <div style={{ background:'rgba(255,255,255,0.05)',borderRadius:14,padding:14,marginBottom:16,minHeight:300,maxHeight:420,overflowY:'auto',display:'flex',flexDirection:'column',gap:10 }}>
        ...
        {supportLoading && <div style={{ fontSize:13,color:'rgba(255,255,255,0.4)',fontFamily:'DM Sans,sans-serif' }}>Isla is typing…</div>}
      </div>
      <div style={{ display:'flex',flexWrap:'wrap',gap:7,marginBottom:12 }}>
        {['Track my order','Refund request','Wrong item','Cancel order','Delivery address help'].map(s=>(
          <button key={s} onClick={()=>handleSupport(s)} style={{ padding:'7px 12px',background:'rgba(255,255,255,0.08)',border:'0.5px solid rgba(255,255,255,0.15)',borderRadius:20,fontSize:11,color:'rgba(255,255,255,0.8)',cursor:'pointer',fontFamily:'DM Sans,sans-serif' }}>{s}</button>
        ))}
      </div>
      <div style={{ display:'flex',gap:8 }}>
        <input value={supportInput} onChange={e=>setSupportInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSupport()} placeholder="Ask Isla anything…" style={{ flex:1,padding:'11px 14px',background:'rgba(255,255,255,0.08)',border:'0.5px solid rgba(255,255,255,0.15)',borderRadius:24,fontFamily:'DM Sans,sans-serif',fontSize:13,color:'white',outline:'none' }}/>
        <button onClick={()=>handleSupport()} style={{ width:42,height:42,background:'#C4683A',border:'none',borderRadius:'50%',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
      ...
      <div style={{ marginTop:14,padding:'12px 14px',background:'rgba(255,255,255,0.05)',borderRadius:10,fontSize:12,color:'rgba(255,255,255,0.5)',fontFamily:'DM Sans,sans-serif',textAlign:'center' }}>
        Can't resolve your issue? Email us at<br/><a href="mailto:support@isladrop.com" style={{ color:'#E8A070' }}>support@isladrop.com</a>
      </div>
    </div>
  )
    ...
    <div style={{ padding:'20px 16px' }}>
      ...
      {user ? (
        <>
          ...
          {[['📦','Order history'],['📍','Saved addresses'],['🔔','Notifications'],['🌍','Language & region']].map(([icon,label])=>(
