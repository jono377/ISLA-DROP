...
function PromotionSettings() {
  const [settings, setSettings] = useState({
    ...
    first_order_discount_code:    'WELCOME10',
  })
    ...
    const load = async () => {
      try {
        ...
        const { data } = await supabase
          .from('app_settings')
          .select('key,value')
          .in('key', ['first_order_discount_enabled','first_order_discount_amount','first_order_discount_code'])
        if (data?.length) {
          const map = {}
          data.forEach(r => {
            map[r.key] = r.key.endsWith('_enabled') ? r.value === 'true' : r.value
          })
          setSettings(s => ({ ...s, ...map }))
        }
      } catch {}
  ...
  const toggleDiscount = () => {
    const next = !settings.first_order_discount_enabled
    save('first_order_discount_enabled', next)
  }

  const DS = { background:'#1A1A2E', color:'white', fontFamily:'DM Sans,sans-serif' }
  const C  = { accent:'#C4683A', green:'#4CAF7D', surface:'rgba(255,255,255,0.05)', border:'rgba(255,255,255,0.1)', muted:'rgba(255,255,255,0.45)' }

  if (loading) return <div style={{ padding:24, color:C.muted, fontSize:13 }}>Loading settings...</div>

  return (
    <div style={{ background:C.surface, border:'0.5px solid '+C.border, borderRadius:14, padding:20, marginBottom:20 }}>
      <div style={{ fontSize:16, fontWeight:600, color:'white', marginBottom:4 }}>Promotion Settings</div>
      <div style={{ fontSize:12, color:C.muted, marginBottom:16 }}>
        Enable or disable customer-facing promotional features. Changes take effect immediately.
      </div>

      {/* First order discount toggle */}
      <div style={{ display:'flex', alignItems:'flex-start', gap:14, padding:'14px 0', borderTop:'0.5px solid '+C.border }}>
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:12, color:C.muted, lineHeight:1.5, marginBottom:6 }}>
            Automatically shows new customers a welcome discount on their first order.
            Currently: <strong style={{ color: settings.first_order_discount_enabled ? C.green : '#E8A070' }}>
              {settings.first_order_discount_enabled ? 'ON' : 'OFF'}
            </strong>
          </div>
          {settings.first_order_discount_enabled && (
            <div style={{ display:'flex', gap:10, alignItems:'center', marginTop:8 }}>
              <div>
                <div style={{ fontSize:11, color:C.muted, marginBottom:4 }}>Discount amount (€)</div>
                <input
                  type="number" min="1" max="50"
                  value={settings.first_order_discount_amount}
                  onChange={e => setSettings(s => ({ ...s, first_order_discount_amount: e.target.value }))}
                  onBlur={e => save('first_order_discount_amount', e.target.value)}
                  style={{ width:80, padding:'7px 10px', background:'rgba(255,255,255,0.08)', border:'0.5px solid rgba(255,255,255,0.15)', borderRadius:8, color:'white', fontSize:14, outline:'none' }}
                />
              </div>
              <div>
                <div style={{ fontSize:11, color:C.muted, marginBottom:4 }}>Discount code</div>
                <input
                  value={settings.first_order_discount_code}
                  onChange={e => setSettings(s => ({ ...s, first_order_discount_code: e.target.value }))}
                  onBlur={e => save('first_order_discount_code', e.target.value.toUpperCase())}
                  style={{ width:140, padding:'7px 10px', background:'rgba(255,255,255,0.08)', border:'0.5px solid rgba(255,255,255,0.15)', borderRadius:8, color:'white', fontSize:13, fontFamily:'monospace', outline:'none', textTransform:'uppercase' }}
                />
              </div>
            </div>
          )}
          {!settings.first_order_discount_enabled && (
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:4 }}>
              💡 Activate when ready — great for growing your early customer base
            </div>
          )}
        </div>
        <button
          ...
          disabled={saving}
          style={{ width:52, height:28, borderRadius:14, background:settings.first_order_discount_enabled?C.green:'rgba(255,255,255,0.15)', border:'none', cursor:'pointer', position:'relative', transition:'background 0.2s', flexShrink:0, marginTop:2 }}>
          <div style={{ width:22, height:22, borderRadius:'50%', background:'white', position:'absolute', top:3, left:settings.first_order_discount_enabled?26:3, transition:'left 0.2s' }}/>
        </button>
      </div>
    </div>
  )
