  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ display: 'flex', background: '#F5F0E8', borderBottom: '0.5px solid rgba(42,35,24,0.12)' }}>
        {['overview', 'orders', 'fleet', 'map'].map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
...
function MapTab({ drivers, orders }) {
  ...
  useEffect(() => {
    ...
    const onlineDrivers = drivers.filter(d => d.is_online && d.current_location)
    ...
    onlineDrivers.forEach(driver => {
      ...
      if (markers.current[driver.id]) {
        markers.current[driver.id].setLngLat([lng, lat])
      } else {
        const el = document.createElement('div')
        el.innerHTML = '<div style="font-size:20px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.2))">🛵</div>'
        markers.current[driver.id] = new mapboxgl.Marker({ element: el })
          .setLngLat([lng, lat])
          .setPopup(new mapboxgl.Popup({ offset: 20 }).setHTML(`<strong>${driver.profiles?.full_name}</strong>`))
          .addTo(map.current)
      }
    })
  }, [drivers])
  ...
  useEffect(() => {
    ...
    orders.forEach(order => {
      ...
      if (!markers.current[key]) {
        const el = document.createElement('div')
        el.innerHTML = '<div style="font-size:16px;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.3))">📍</div>'
        markers.current[key] = new mapboxgl.Marker({ element: el })
          .setLngLat([order.delivery_lng, order.delivery_lat])
          .setPopup(new mapboxgl.Popup({ offset: 16 }).setHTML(`<strong>#${order.order_number}</strong><br>${order.delivery_address}`))
          .addTo(map.current)
      }
    })
  }, [orders])
  ...
  return (
    <div style={{ margin: '0 -16px' }}>
      <div style={{ padding: '0 16px 12px', display: 'flex', gap: 16, fontSize: 12, color: '#7A6E60' }}>
        <span>🛵 Drivers online</span>
        <span>📍 Active orders</span>
      </div>
      <div ref={mapContainer} style={{ height: 460 }} />
    </div>
  )
}
