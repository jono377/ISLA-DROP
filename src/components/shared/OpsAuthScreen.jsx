...
export default function OpsAuthScreen() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (profile?.role !== 'ops') {
        ...
        toast.error('Access denied — ops staff only')
        setLoading(false)
        return
      }
      ...
      setUser(user)
      setProfile(profile)
      toast.success('Welcome to Ops Dashboard')
    } catch (err) {
    ...
    <div style={{ minHeight:'100vh', background:'#0D3545', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ width:'100%', maxWidth:380 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:36 }}>
          ...
          <div style={{ fontSize:13, color:'rgba(255,255,255,0.45)', letterSpacing:'1.5px', textTransform:'uppercase' }}>Operations Dashboard</div>
        </div>
          ...
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div>
              ...
              <input type="email" value={form.email} onChange={handle('email')}
                placeholder="ops@isladrop.net" required autoComplete="email"
                style={inp} />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading}
              style={{ width:'100%', padding:'14px', marginTop:4, background:loading?'rgba(196,104,58,0.5)':'#C4683A', border:'none', borderRadius:12, color:'white', fontSize:15, fontWeight:600, cursor:loading?'default':'pointer', fontFamily:'DM Sans,sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              {loading
                ? <><Spinner /> Signing in...</>
                : 'Sign in to Ops →'
              }
        ...
        <div style={{ textAlign:'center', marginTop:20, fontSize:12, color:'rgba(255,255,255,0.2)', fontFamily:'DM Sans,sans-serif' }}>
          Restricted access · Isla Drop staff only
        </div>
      </div>
    </div>
  )
...
function Spinner() {
  return (
    <div style={{ width:16, height:16, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'white', borderRadius:'50%', animation:'opsSpin 0.8s linear infinite' }}>
